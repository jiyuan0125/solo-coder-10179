...
Errors.getPosition = function(error, tokenIndex) {
    ...
    if (!pos) {
        // Fallback: use cst library's element location.
        // This provides a reliable default when tokenIndex hasn't
        // indexed the element (e.g. parse-error sentinel elements).
        pos = element.getLoc ? element.getLoc().start : EMPTY_POS;
    }
    if (!pos) {
        return EMPTY_POS;
    }
