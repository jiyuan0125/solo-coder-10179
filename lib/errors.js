...
Errors.prototype = {
    ...
    /**
     * Adds a pre-computed error to the list.
     * Used when error position is already calculated (e.g., for extracted code).
     *
     * @param {Object} error
     * @param {String} error.filename
     * @param {String} error.rule
     * @param {String} error.message
     * @param {Number} error.line
     * @param {Number} error.column
     */
    addRaw: function(error) {
    ...
    explainError: function(error, colorize) {
        var prevEnabled = chalk.enabled;
        if (colorize && !chalk.enabled) {
            chalk.enabled = true;
        }
