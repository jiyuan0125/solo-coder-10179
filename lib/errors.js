...
function assertOffsetValue(offset) {
    if (typeof offset !== 'number' || isNaN(offset) || offset < 0) {
        throw new Error('Offset must be a non-negative number');
    }
}

Errors.registerRuleDefaultOffset = function(ruleName, offset) {
    assertRuleName(ruleName);
    assertOffsetValue(offset);
    _ruleOffsetRegistry[ruleName] = offset;
};

Errors.getRuleDefaultOffset = function(ruleName) {
    return _ruleOffsetRegistry.hasOwnProperty(ruleName) ?
        _ruleOffsetRegistry[ruleName] : undefined;
};

Errors.registerRuleDefaultOffset('validateQuoteMarks', 0);
 ...
 *
 * Single source of truth: __loc written onto tokens by TokenIndex._buildIndex.
 * If a TokenIndex is provided it is used to resolve the element's position;
 * otherwise the element's own __loc (or its first token's __loc) is used
 * directly. There is intentionally NO fallback to the cst library's
 * element.getLoc() — keeping a single source avoids drift between the two.
 *
 * @param {Error} error
 * @param {TokenIndex} [tokenIndex]
 * @return {Object}
...
Errors.getPosition = function(error, tokenIndex) {
    ...
    if (offset === undefined) {
        var registeredOffset = Errors.getRuleDefaultOffset(rule);
        if (registeredOffset !== undefined) {
    ...
    var pos;
    if (tokenIndex) {
        pos = tokenIndex.getElementLoc(element);
    } else if (element.__loc) {
        pos = element.__loc;
    } else if (typeof element.getFirstToken === 'function') {
        var firstTok = element.getFirstToken();
        pos = firstTok && firstTok.__loc;
    }
