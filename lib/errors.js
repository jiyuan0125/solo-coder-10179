...
Errors.prototype = {
    ...
    calculateErrorLocations: function(tokenIndex) {
        var self = this;
        this._errorList.forEach(function(error, index) {
            var pos = Errors.getPosition(error, tokenIndex);
            self.updateErrorPosition(index, pos.line, pos.column);
        });
    },

    /**
     * Public API to update error position.
     *
     * @param {Number} index
     * @param {Number} line
     * @param {Number} column
     */
    updateErrorPosition: function(index, line, column) {
        this._errorList[index].line = line;
        this._errorList[index].column = column;
    },

    /**
     * Public API to add an externally created error (e.g., from HTML extraction).
     *
     * @param {Object} errorData
     * @param {String} errorData.filename
     * @param {String} errorData.rule
     * @param {String} errorData.message
     * @param {Number} errorData.line
     * @param {Number} errorData.column
     */
    addExternalError: function(errorData) {
        this._errorList.push({
    ...
    /**
     * Public API to apply coordinate transformation to all errors.
     * Used for HTML embedded script error position translation.
     *
     * @param {Number} lineOffset - line offset to add
     * @param {Number} columnOffset - column offset to add
     */
    applyCoordinateTransform: function(lineOffset, columnOffset) {
        this._errorList.forEach(function(error) {
            error.line += lineOffset;
            error.column += columnOffset;
        });
    },

    /**
     * Public API to mark an error as fixed.
     *
     * @param {Number} index
     */
    markErrorFixed: function(index) {
        this._errorList[index].fixed = true;
    },

    /**
     * Public API to check if an error should appear in final output.
     * An error marked as fixed should NOT appear if the code actually changed.
     * If the rule marked it fixed but didn't actually change the code,
     * it should still appear.
     *
     * @param {Number} index
     * @param {Boolean} codeActuallyChanged - whether the code was modified
     * @returns {Boolean}
     */
    shouldErrorAppearInFinalOutput: function(index, codeActuallyChanged) {
        var error = this._errorList[index];
    ...
    /**
     * Public API to sort errors.
     *
     * @param {Function} comparator
     */
    sortErrors: function(comparator) {
        this._errorList.sort(comparator);
    },
