...
TokenIndex.prototype.getElementLoc = function(element) {
    // Handle the case where element itself is a token (e.g. rules that
    // pass tokens directly to errors.add() / errors.cast()).
    // Fall back to element's own __loc if it has been indexed,
    // otherwise walk tokens to find a match by object identity.
    var token = element.getFirstToken ? element.getFirstToken() : element;
    if (token && token.__loc) {
        return token.__loc;
    }
    // Fallback: search indexed tokens by identity
    for (var i = 0; i < this._tokens.length; i++) {
        if (this._tokens[i] === token || this._tokens[i] === element) {
