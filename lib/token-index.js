 ...
 *
 * Single source of truth: the __loc property written onto tokens during
 * _buildIndex. If element is itself a token, return its __loc directly.
 * If element is a syntax node, resolve its first token and return that
 * token's __loc. If __loc is not set for any token in the lookup chain,
 * walk _tokens to find a match by object identity. If absolutely nothing
 * is found, return {line: 1, column: 0} — the caller can use EMPTY_POS
 * equality to decide whether to discard this error.
 *
 * @param {Element} element
 * @returns {Object}
 */
TokenIndex.prototype.getElementLoc = function(element) {
    ...
    // Case 1: element itself is an indexed token (e.g. rules pass tokens directly)
    if (element && element.__loc) {
        return element.__loc;
    }

    // Case 2: element is a syntax node; resolve its first token
    if (element && typeof element.getFirstToken === 'function') {
        firstToken = element.getFirstToken();
        if (firstToken && firstToken.__loc) {
            return firstToken.__loc;
        }
    }

    // Case 3: fallback — search indexed tokens by object identity
    if (element) {
        for (var i = 0; i < this._tokens.length; i++) {
            if (this._tokens[i] === element ||
                (firstToken !== undefined && this._tokens[i] === firstToken)) {
                return this._tokens[i].__loc;
            }
        }
    }
