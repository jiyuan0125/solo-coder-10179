 ...
 *
 * Uses an internal WeakMap to store token locations instead of
 * polluting external CST token objects with __loc property.
 *
 * @param {Element} firstToken
 * @constructor
 */
function TokenIndex(firstToken) {
    this._tokenLocs = new WeakMap();
...
TokenIndex.prototype._buildIndex = function(firstToken) {
    ...
    while (currentToken) {
        ...
        this._tokenLocs.set(currentToken, previousLoc);
...
TokenIndex.prototype.getElementLoc = function(element) {
    ...
    return this._tokenLocs.get(token) || {
        line: 1,
        column: 0
    };
};
