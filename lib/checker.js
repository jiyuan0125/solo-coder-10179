...
Checker.prototype.extractFile = function(path) {
    ...
    return vowFs.read(path, 'utf8').then(function(data) {
        ...
        result.sources.forEach(function(script) {
            var scriptErrors = this.checkString(script.source, path);
            scriptErrors.applyCoordinateTransform(script.line, script.offset);
            scriptErrors.getErrorList().forEach(function(error) {
