...
StringChecker.prototype._initRunState = function() {
    this._runState = {
        errorsFound: 0,
    ...
    _checkJsFile: function(file, errors) {
        ...
        if (this.maxErrorsEnabled()) {
            if (this._maxErrors === -1 || this._maxErrors === null) {
                ...
                this._runState.maxErrorsExceeded = this._runState.errorsFound + errors.getErrorCount() > this._maxErrors;
                errors.stripErrorList(Math.max(0, this._maxErrors - this._runState.errorsFound));
            }
        }
