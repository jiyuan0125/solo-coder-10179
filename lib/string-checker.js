...
StringChecker.prototype = {
    ...
    configure: function(config) {
        ...
        // Reset per-configure run state to avoid cross-configure state leaks
        // in long-running processes (daemon, watch mode).
        this._initRunState();
    },
    ...
    checkString: function(source, filename) {
        ...
        // Ensure run state is initialized (defensive in case configure was not called)
        if (!this._runState) {
            this._initRunState();
        }
    ...
    runReporter: function(name, errorsCollection) {
        ...
        if (typeof reporter === 'function') {
            // Detect legacy reporters: those that expect raw errorsCollection
            // are identified by not explicitly requesting the context object.
            // For backward compatibility, pass raw errorsCollection if the
            // function is not explicitly designed for the new contract.
            // Since we can't reliably detect intent, we support both:
            // - If reporter.length === 1, assume new contract (pass context)
            // - Otherwise, assume legacy mode (pass errorsCollection directly)
