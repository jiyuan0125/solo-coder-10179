...
/**
 * Public API: Transforms an error's coordinates from within an extracted
 * script to the correct coordinates in the original HTML file.
 *
 * This is the formal, supported API for HTML coordinate transformation.
 * Do NOT manually adjust error.line and error.column - use this function.
 *
 * @param {Object} error - Error object from checking an extracted script
 * @param {Object} scriptInfo - Script metadata { line: Number, offset: Number }
 * @returns {Object} New error object with transformed coordinates
 */
function transformErrorCoordinates(error, scriptInfo) {
    return {
        filename: error.filename,
        rule: error.rule,
        message: error.message,
        line: error.line + scriptInfo.line - 1,
        column: error.column + scriptInfo.offset
    };
}
...
function extractJs(filename, data) {
    ...
    return {
        ...
        errors: errors,
        /**
         * Public API for adding an error with proper coordinate transformation.
         * Uses the unified addRawError API instead of directly accessing _errorList.
         *
         * @param {Object} error - Error from the extracted script
         * @param {Object} scriptInfo - Script metadata for coordinate transformation
         */
        addError: function(error, scriptInfo) {
            var transformedError = transformErrorCoordinates(error, scriptInfo);
            errors.addRawError(transformedError);
        }
    };
}

// Export the coordinate transform as a public API
extractJs.transformErrorCoordinates = transformErrorCoordinates;
extractJs.getScripts = getScripts;
extractJs.normalizeSource = normalizeSource;
