...
// Sentinel token type used by cst library when creating a fallback Program
// after a parse failure. If cst changes its fallback strategy in the future,
// only this constant needs updating; business logic ("is this an empty file?")
// doesn't need to change.
var PARSE_ERROR_SENTINEL_TYPE = 'EOF';
...
var JsFile = function(params) {
    ...
    try {
        ...
        this._isEmpty = this._computeIsEmpty();
        // Eagerly build a TokenIndex to write __loc onto every token. This
        // makes token positions available to anyone holding a token reference
        // (e.g. Errors.getPosition called without a TokenIndex, TokenAssert,
        // any rule that wants to inspect a token's line/column directly).
        new TokenIndex(this._program.getFirstToken());
    } catch (e) {
        this._parseErrors.push(e);
        this._program = new Program([
            new Token(PARSE_ERROR_SENTINEL_TYPE, '')
        ]);
        // Parse failure always produces a synthetic empty Program; treat it
        // as empty so callers don't try to run rules over it.
        this._isEmpty = true;
        // Still seed __loc on the synthetic EOF token so position lookups
        // don't fall over.
        var firstToken = this._program.getFirstToken();
        if (firstToken) {
            firstToken.__loc = { line: 1, column: 0 };
        }
    }
...
JsFile.prototype = {
    ...
    /**
     * Returns true if the file contains no meaningful code
     * (e.g. empty source, parse-failure sentinel).
     * Decouples callers from the internal representation of emptiness.
     *
     * @returns {Boolean}
     */
    isEmpty: function() {
    ...
    /**
     * Computes whether a successfully-parsed file contains no meaningful code
     * tokens (i.e. only whitespace, comments, or cst-library sentinel tokens).
     *
     * This method is intentionally decoupled from cst library internals:
     * it does NOT check for sentinel node types like 'EOF' by literal
     * string comparison against external type constants. Parse-failure
     * emptiness is handled directly in the constructor catch block.
     * If cst changes its empty-file representation in the future, only
     * this one helper needs updating.
     *
     * @private
     * @returns {Boolean}
     */
    _computeIsEmpty: function() {
        // Fast path: source with no non-whitespace characters is empty.
        if (typeof this._source === 'string' && this._source.trim().length === 0) {
            return true;
        }
        if (!this._program) {
            return true;
        ...
        // Walk tokens: if we find any meaningful token (not whitespace,
        // not comment), the file is not empty.
        while (token) {
            var isMeaningful = !token.isWhitespace &&
                               !token.isComment;
            if (isMeaningful) {
                // A single meaningful token means the file is not empty.
                // We deliberately do NOT compare against cst's internal
                // sentinel type strings here.
                return false;
            }
            token = token.getNextToken();
        }
