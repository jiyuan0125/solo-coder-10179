...
var JsFile = function(params) {
    ...
    // Per-instance storage for token locations (avoids polluting CST tokens)
    this._tokenLocations = new WeakMap();
};
...
JsFile.prototype = {
    ...
    /**
     * Public API to check if this file is empty (no AST or only EOF token).
     * Use this instead of directly accessing file._program.
     *
     * @returns {Boolean}
     */
    isEmpty: function() {
        return !this._program || this._program.firstChild.type === 'EOF';
    },

    /**
     * Stores location information for a token without polluting the token object.
     * Part of the unified position tracking API.
     *
     * @param {Object} token - CST token
     * @param {Object} loc - Location {line, column}
     */
    setTokenLoc: function(token, loc) {
        this._tokenLocations.set(token, loc);
    },

    /**
     * Retrieves stored location information for a token.
     * Part of the unified position tracking API.
     *
     * @param {Object} token - CST token
     * @returns {Object|undefined} Location {line, column} or undefined
     */
    getTokenLoc: function(token) {
        return this._tokenLocations.get(token);
    },
